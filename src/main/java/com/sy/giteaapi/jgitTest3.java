...
public class jgitTest3 {
	...
	//定义本地git路径
    public static final String LOCALPATH = "D:/project/";
    //.git文件路径
...
.setDirectory(new File(LOCALPATH)).call();
  ...
  //pull拉取远程仓库文件
	/*
	 * public static boolean pullBranchToLocal(){ boolean resultFlag = false;
	 * //git仓库地址 Git git; try { git = new Git(new FileRepository(LOCALGITFILE));
	 * git.pull().setRemoteBranchName("master") .setCredentialsProvder(new
	 * UsernamePasswordCredentialsProvider(USER,PASSWORD)).call(); resultFlag =
	 * true; } catch (IOException | GitAPIException e) { e.printStackTrace(); }
	 * return resultFlag; }
	 */
